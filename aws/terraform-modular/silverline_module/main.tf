...
resource "null_resource" "login" {
  ...
  provisioner "local-exec" {
    command = "curl -k -X POST -H 'Content-type: application/json' --data-raw '${data.template_file.login.rendered}' ${var.silverline_login_url} > ${path.module}/dynamic_docs/f5sl1_login_response.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5sl1_login_response.json"
  }
}

#  Open the login_response file
data "local_file" "login_response" {
  filename   = "${path.module}/dynamic_docs/f5sl1_login_response.json"
...
#   provisioner "local-exec" {
#     command = "curl -k -X GET -H 'Content-type: application/json' -H 'X-Authorization-Token: ${local.token}' ${var.silverline_proxies_url} > ${path.module}/dynamic_docs/f5sl2_get_proxies_response.json"
#   }

#   provisioner "local-exec" {
#     when    = destroy
#     command = "rm -f ${path.module}/dynamic_docs/f5sl2_get_proxies_response.json"
#   }
# }
...
resource "local_file" "rendered_sl_certs" {
  ...
  filename = "${path.module}/dynamic_docs/f5sl3_rendered_create_certs_${count.index}.json"
}
...
resource "null_resource" "create_certs" {
  ...
  provisioner "local-exec" {
    command = "curl --location --request POST -H 'Content-type: application/json' -H 'X-Authorization-Token: ${local.token}' --data-binary '@${path.module}/dynamic_docs/f5sl3_rendered_create_certs_${count.index}.json' ${var.silverline_certs_url} > ${path.module}/dynamic_docs/f5sl4_create_certs_response_${count.index}.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5sl4_create_certs_response_${count.index}.json"
  }
}
...
resource "local_file" "rendered_sl_front_end" {
  ...
  filename = "${path.module}/dynamic_docs/f5sl5_rendered_create_front_end_${count.index}.json"
}
...
resource "null_resource" "create_front_end" {
  ...
  provisioner "local-exec" {
    command = "curl -v --location --request POST -H 'Content-type: application/json' -H 'X-Authorization-Token: ${local.token}' --data-binary '@${path.module}/dynamic_docs/f5sl5_rendered_create_front_end_${count.index}.json' ${var.silverline_front_end_profile_url} > ${path.module}/dynamic_docs/f5sl6_create_front_end_response_${count.index}.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5sl6_create_front_end_response_${count.index}.json"
  }
}
...
resource "local_file" "rendered_sl_proxy" {
  ...
  filename = "${path.module}/dynamic_docs/f5sl7_rendered_create_proxy_${count.index}.json"
}
...
resource "null_resource" "create_proxy" {
  ...
  provisioner "local-exec" {
    command = "sleep 20; curl --location --request POST -H 'Content-type: application/json' -H 'X-Authorization-Token: ${local.token}' --data-binary '@${path.module}/dynamic_docs/f5sl7_rendered_create_proxy_${count.index}.json' ${var.silverline_proxies_url} > ${path.module}/dynamic_docs/f5sl8_create_proxy_response_${count.index}.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5sl8_create_proxy_response_${count.index}.json"
  }
}
...
data "local_file" "create_proxy_response" {
  count      = var.app_count
  filename   = "${path.module}/dynamic_docs/f5sl8_create_proxy_response_${count.index}.json"
...
resource "null_resource" "deploy_proxy" {
  ...
  provisioner "local-exec" {
    command = "curl --location --request POST -H 'Content-type: application/json' -H 'X-Authorization-Token: ${local.token}' --data-binary '@${path.module}/templates/deploy_proxy_json.tpl' ${var.silverline_proxies_url}/${jsondecode(data.local_file.create_proxy_response[count.index].content).data.id}/deployments > ${path.module}/dynamic_docs/f5sl9_deploy_proxy_response_${count.index}.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5sl9_deploy_proxy_response_${count.index}.json"
  }
}