...
resource "null_resource" "cs_find_dns_subscription" {
  ...
  provisioner "local-exec" {
    ...
    command = "curl -k -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' https://api.cloudservices.f5.com/v1/svc-subscription/subscriptions?account_id=${var.f5cs_account_id} > ${path.module}/dynamic_docs/f5cs7_get_subs_list.json"
  }
