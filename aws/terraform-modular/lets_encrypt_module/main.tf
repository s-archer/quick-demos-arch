...
resource "local_file" "rendered_as3_with_challenge_iRule" {
  ...
  filename = "${path.module}/declarations/rendered_as3_with_iRule_${count.index}.json"

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/declarations/rendered_as3_with_iRule_${count.index}.json"
  }
}
...
resource "null_resource" "as3_with_iRule" {
  ...
  provisioner "local-exec" {
    command = "curl -k -u ${var.f5_user}:${var.f5_pass[count.index]} -X POST -H 'Content-type: application/json' --data-binary \"@${path.module}/declarations/rendered_as3_with_iRule_${count.index}.json\" https://${var.mgmt_ips[count.index]}/mgmt/shared/appsvcs/declare"
  }
...
resource "local_file" "rendered_as3_with_signed_certs" {
  ...
  filename = "${path.module}/declarations/rendered_as3_with_signed_certs_${count.index}.json"

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/declarations/rendered_as3_with_signed_certs_${count.index}.json"
  }
}
...
resource "null_resource" "as3_with_with_signed_certs" {
  ...
  provisioner "local-exec" {
    command = "curl -k -u ${var.f5_user}:${var.f5_pass[count.index]} -X POST -H 'Content-type: application/json' --data-binary \"@${path.module}/declarations/rendered_as3_with_signed_certs_${count.index}.json\" https://${var.mgmt_ips[count.index]}/mgmt/shared/appsvcs/declare"
  }
}