        ...
        "gslb_service": {
			...
			"pools": {
				format("%s", gslb_zone_a_record ) : {
					...
					"max_answers": 1,
					"members": [for index, each_site in pub_vs_eips_list :
						{
			...
			"virtual_servers": { for index, each_site in pub_vs_eips_list :
				format("%s_%s", gslb_zone_a_record , index ) => {
					"virtual_server_type": "cloud",
					"display_name": each_site.public_dns,
					"port": 443,
					"address": each_site.public_ip,
					"monitor": "tcp_monitor"
				}
			},
			"monitors": {
