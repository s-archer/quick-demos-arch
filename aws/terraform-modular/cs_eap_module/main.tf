...
resource "local_file" "rendered_cs_certs" {
  ...
  filename = "${path.module}/dynamic_docs/f5cs4_rendered_cs_add_cert_${count.index}.json"
}
...
resource "null_resource" "cs_create_certs" {
  ...
  provisioner "local-exec" {
    command = "curl -k -X POST -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' --data-binary '@${path.module}/dynamic_docs/f5cs4_rendered_cs_add_cert_${count.index}.json' https://api.cloudservices.f5.com/v1/svc-certificates/certificates > ${path.module}/dynamic_docs/f5cs5_create_certs_${count.index}.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5cs5_create_certs_${count.index}.json"
  }
}
...
data "local_file" "f5cs5_create_certs" {
  count      = var.app_count
  filename   = "${path.module}/dynamic_docs/f5cs5_create_certs_${count.index}.json"
...
resource "local_file" "rendered_cs_eap" {
  ...
  filename = "${path.module}/dynamic_docs/f5cs6_rendered_cs_eap_${count.index}.json"
}
...
resource "null_resource" "cs_create_eap" {
  ...
  provisioner "local-exec" {
    command = "curl -k -X POST -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' --data-binary '@${path.module}/dynamic_docs/f5cs6_rendered_cs_eap_${count.index}.json' https://api.cloudservices.f5.com/v1/svc-subscription/subscriptions > ${path.module}/dynamic_docs/f5cs7_create_eap_${count.index}.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5cs7_create_eap_${count.index}.json"
  }
}
...
data "local_file" "cs_create_eap" {
  count      = var.app_count
  filename   = "${path.module}/dynamic_docs/f5cs7_create_eap_${count.index}.json"
...
resource "null_resource" "cs_activate_eap" {
  ...
  provisioner "local-exec" {
    command = "curl -k -X POST -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' https://api.cloudservices.f5.com/v1/svc-subscription/subscriptions/${local.eap_subs_id[count.index]}/activate > ${path.module}/dynamic_docs/f5cs8_activate_eap_${count.index}.json"
  }
  ...
  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5cs8_activate_eap_${count.index}.json"
  }
}
...
resource "null_resource" "cs_get_subs_cname" {
  ...
  provisioner "local-exec" {
    command = "curl -k -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' https://api.cloudservices.f5.com/v1/svc-subscription/subscriptions/${local.eap_subs_id[count.index]} > ${path.module}/dynamic_docs/f5cs9_get_subs_cname_${count.index}.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5cs9_get_subs_cname_${count.index}.json"
  }
}

data "local_file" "cs_get_subs_cname" {
  count      = var.app_count
  filename   = "${path.module}/dynamic_docs/f5cs9_get_subs_cname_${count.index}.json"
...
resource "local_file" "rendered_cs_eap_cname" {
  ...
  filename = "${path.module}/dynamic_docs/f5cs10_rendered_cs_eap_cname.json"
}
...
resource "null_resource" "cs_find_dns_subscription" {
  ...
  provisioner "local-exec" {
    #command = "curl -k -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' https://api.cloudservices.f5.com/v1/svc-subscription/subscriptions?account_id=${local.mem_acc_id[0]} > ${path.module}/dynamic_docs/f5cs11_get_subs_list.json"
    command = "curl -k -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' https://api.cloudservices.f5.com/v1/svc-subscription/subscriptions?account_id=${var.f5cs_account_id} > ${path.module}/dynamic_docs/f5cs11_get_subs_list.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5cs11_get_subs_list.json"
  }
}


data "local_file" "f5cs_get_subs_list_response" {
  filename   = "${path.module}/dynamic_docs/f5cs11_get_subs_list.json"
...
resource "null_resource" "cs_modify_eap_cname_in_parent_zone" {
  ...
  provisioner "local-exec" {
    command = "curl -k -X PUT -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' --data-binary '@${path.module}/dynamic_docs/f5cs10_rendered_cs_eap_cname.json' https://api.cloudservices.f5.com/v1/svc-subscription/subscriptions/${local.dns_subs[0]} > ${path.module}/dynamic_docs/f5cs12_modify_eap_cname_in_parent_zone_response.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/dynamic_docs/f5cs12_modify_eap_cname_in_parent_zone_response.json"
  }
}